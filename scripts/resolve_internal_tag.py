...
def validate_inputs(
    ...
    force_repromote,
    recover_tag,
    recover_candidate_digest,
):
    digests = (upstream_digest, selected_child_digest)
    if not upstream_tag or any(
        not isinstance(value, str) or not DIGEST_RE.fullmatch(value)
        for value in digests
    ):
        raise ValueError(
            "upstream tag, index digest, and selected child digest must be valid"
        )
    if upstream_digest == selected_child_digest:
        raise ValueError(
            "upstream index digest must differ from the selected child digest"
        )
    ...
    if bool(recover_tag) != bool(recover_candidate_digest):
        raise ValueError(
            "recover_tag and recover_candidate_digest must be supplied together"
        )
...
def validate_observation(item, pattern, upstream_tag):
    if not isinstance(item, dict) or set(item) != OBSERVATION_FIELDS:
        raise ValueError(
            "each observation must contain exactly the verified identity fields"
        )
    ...
    if (
        ...
        raise ValueError(
            "each observation must contain a valid tag and candidate digest"
        )
    ...
    if not match:
        raise ValueError(
            f"observation tag does not match {upstream_tag}-bocklabs.N: {tag}"
        )
    if child is not None:
        validate_phase05_observation(tag, index, child, platforms)
    elif index not in ("", None):
        raise ValueError(
            f"historical observation cannot carry a Phase 05 index digest: {tag}"
        )
    return tag, match
...
def recovered_revision(
    candidates, recover_tag, selected_child_digest, recover_candidate_digest
):
    ...
    if observed["digest"] != recover_candidate_digest:
        raise ValueError(
            "recover_tag digest does not match the trusted candidate digest"
        )


def allocated_revision(
    upstream_tag, ordered, upstream_digest, selected_child_digest, force_repromote
):
    ...
    if not force_repromote:
        matching = [
            item
            for item in phase05
            if item["selected_child_digest"] == selected_child_digest
        ]
        if matching:
            reused = matching[0]
            return (
                reused["tag"],
                True,
                reused["upstream_index_digest"] != upstream_digest,
                False,
            )
    ...
    higher_upstream = (
        ...
        and comparison["upstream_index_digest"] != upstream_digest
    )
    original_child_selected = (
        comparison is not None
        and comparison["selected_child_digest"] != selected_child_digest
    )
    return (
        f"{upstream_tag}-bocklabs.{next_revision}",
        False,
        higher_upstream,
        original_child_selected,
    )
...
def select_internal_tag(
    ...
    ordered = sorted(
        candidates.values(), key=lambda item: item["revision"], reverse=True
    )
    if recover_tag:
        recovered_revision(
            candidates, recover_tag, selected_child_digest, recover_candidate_digest
...
def main() -> int:
    ...
    try:
        ...
        internal_tag, skip_copy, higher_upstream, original_child_selected = (
            select_internal_tag(
                args.upstream_tag,
