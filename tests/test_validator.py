...
class ValidatorTests(unittest.TestCase):
    ...
    def test_upstream_action_inputs_reject_shell_syntax(self):
        for field, value in (
            ...
            with self.subTest(field=field):
                ...
                self.assert_fails(entry, "shell-safe", f"spec.upstream.{field}")
    ...
    def test_disabled_patch_policy_requires_structured_reason(self) -> None:
        entry = valid_entry()
        entry["spec"]["patchPolicy"] = "disabled"
        self.assert_fails(entry, "spec.patchDisabledReason is required")
